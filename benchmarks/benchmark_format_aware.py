...
def benchmark_operation(name: str, cpu_func, gpu_func, data: GSData, device: str,
...
def main():
    ...
    for n in sizes:
        ...
        results.append(benchmark_operation(
            ...
            lambda g: ColorGPU().brightness(1.1).contrast(1.05).saturation(1.2)(g, inplace=True),
            data, device
        ))
        ...
        results.append(benchmark_operation(
            "transform_pipeline",
            lambda d: Transform().translate([1, 0, 0]).scale(1.5).rotate_axis_angle([0, 1, 0], 0.3)(d, inplace=True),
            lambda g: TransformGPU().translate([1, 0, 0]).scale(1.5).rotate_axis_angle([0, 1, 0], 0.3)(g, inplace=True),
            data, device
        ))
        ...
        # Print results table
        print("\n" + "-" * 70)
        print(f"{'Operation':<25} {'CPU (ms)':<12} {'GPU (ms)':<12} {'Speedup':<10} {'Max Diff':<12}")
        print("-" * 70)

        for r in results:
            max_diff_str = f"{r['max_diff']:.2e}" if r['max_diff'] >= 0 else "ERROR"
            print(f"{r['name']:<25} {r['cpu_ms']:<12.3f} {r['gpu_ms']:<12.3f} {r['speedup']:<10.1f}x {max_diff_str:<12}")
